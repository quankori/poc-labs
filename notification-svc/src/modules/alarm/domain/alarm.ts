import { VersionedAggregateRoot } from 'src/modules/shared/domain/aggregate-root';
import { AlarmItem } from './alarm-item';
import { AlarmSeverity } from './value-objects/alarm-severity';
import { AlarmAcknowledgeEvent } from './events/alarm-acknowledge.event';
import { SerializedEventPayload } from 'src/modules/shared/domain/interfaces/serializable-event';
...
export class Alarm extends VersionedAggregateRoot {
  ...
  [`on${AlarmAcknowledgeEvent.name}`](
    ...
    if (this.isAcknowledged) {
      throw new Error('Alarm has already been Acknowled');
    }
